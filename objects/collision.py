...
def collision(particles, non_gravity_objects):
    ...
    #* a more mathematical variable for Collision Damping
    COLLISION_DAMPING_CONSTANT = lambda mass: (mass/2)
    ...
    for particle in collisions.keys():
        ...
        if particle.rect.y - particle.radius > collisions[particle][0].rect.y:
            particle.velocity[1] = -((v1*(m1 - m2) + 2*m2*v2) / (m1 + m2)) * COLLISION_DAMPING_CONSTANT(particle.mass)
            collisions[particle][0].velocity = [collisions[particle][0].velocity[0], ((v2*(m2-m1) + 2*m1*v1) / (m1+m2)) * COLLISION_DAMPING_CONSTANT(particle.mass)]
        else:
            particle.velocity[1] = ((v1 * (m1 - m2) + 2 * m2 * v2) / (m1 + m2)) * COLLISION_DAMPING_CONSTANT(particle.mass)
            collisions[particle][0].velocity = [collisions[particle][0].velocity[0], ((v2 * (m2 - m1) + 2 * m1 * v1) / (m1 + m2))]
        if particle.rect.x - particle.radius > collisions[particle][0].rect.x:
            ...
            particle.velocity[0] = -((v1*(m1 - m2) + 2*m2*v2) / (m1 + m2)) * COLLISION_DAMPING_CONSTANT(particle.mass)
            collisions[particle][0].velocity = [((v2*(m2-m1) + 2*m1*v1) / (m1+m2)), collisions[particle][0].velocity[1]]
        else:
            v1, v2 = particle.velocity[0], collisions[particle][0].velocity[0]
            particle.velocity[0] = ((v1 * (m1 - m2) + 2 * m2 * v2) / (m1 + m2)) * COLLISION_DAMPING_CONSTANT(particle.mass)
            collisions[particle][0].velocity = [((v2 * (m2 - m1) + 2 * m1 * v1) / (m1 + m2)),collisions[particle][0].velocity[1]]
